...
WHERE d.no_empleado = $docentea -- El ID del rol del coordinador --AND d.no_empleado = @no_empleado_maestro -- Verifica que sea el docente actual
AND NOT EXISTS (
    SELECT 1
    FROM lime_uaim_encuesta e
    WHERE e.id_evaluado = d.no_empleado
    AND e.id_evaluador = $docentea -- Verifica que el maestro actual ya no haya calificado al coordinador
);";
