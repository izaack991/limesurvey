...
WHERE d.es_docente = 1 AND d.no_empleado != $docentejsa -- El ID del rol del coordinador --AND d.no_empleado = @no_empleado_maestro -- Verifica que sea el docente actual
AND NOT EXISTS (
    SELECT 1
    FROM lime_uaim_encuesta e
    WHERE e.id_evaluado = d.no_empleado
    AND e.id_evaluador = $docentejsa
    AND e.id_encuesta = 4 -- Verifica que el maestro actual ya no haya calificado al coordinador
);";
